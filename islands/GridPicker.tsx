...
export default function GridPicker({ setGrid }: GridPickerProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-4 md:gap-2">
      ...
      <div
        ref={gridRef}
        className="grid touch-none select-none grid-cols-7 gap-1.5 md:gap-1"
        ...
        {Array.from({ length: rows }, (_, i) =>
          Array.from({ length: cols }, (_, j) => {
            ...
            return (
              <div
                key={`${row}-${col}`}
                className={`aspect-4/5 cursor-pointer rounded-sm transition hover:scale-105 md:rounded-xs ${bg}`}
                onPointerDown={(e) => {
                  e.preventDefault();
